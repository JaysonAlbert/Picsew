...
- **核心决策**：保留现有反馈入口交互，修复 `Dialog` 包装组件的 `ref` 传递问题，并为对话框增加不依赖当前 Tailwind utility 产物的关键定位样式。
...
- **当前问题**:
  ...
  3. 线上样式产物缺少 `fixed`、`inset-0`、`z-50`、`top-[50%]` 等对话框定位 utility，导致弹窗节点存在但被排版到视口外。
- **目标**:
  1. 确保点击反馈按钮后弹窗稳定打开。
...
- [ ] AC-04：[P1] 在手机视口下点击反馈按钮后，弹窗主内容出现在可视区域内。
...
#### ADR-002: 为 Dialog 关键定位样式增加显式兜底

**状态**：Accepted

**背景**：当前项目提交的 `src/index.css` 和打包后的 CSS 只包含一部分 Tailwind utility。`Dialog` 所依赖的 `fixed`、`inset-0`、`z-50`、`top-[50%]`、`translate-x-[-50%]` 等类名没有在最终产物中生效，导致弹窗 DOM 存在但几何位置落在视口外。

**决策**：在 `DialogOverlay` 与 `DialogContent` 上保留现有 className，同时用显式 `style` 为关键定位、层级和尺寸属性提供兜底，确保当前产物下对话框能正确显示。

**备选方案对比**：

| 方案                                          | 优点                             | 缺点                                                 | 结论 |
| --------------------------------------------- | -------------------------------- | ---------------------------------------------------- | ---- |
| 方案 A：对 Dialog 增加显式关键样式兜底        | 改动小，能快速恢复反馈弹窗可用性 | 没有从根上修复整个 Tailwind 产物缺失问题             | 采用 |
| 方案 B：立即重做整个 Tailwind v4 样式构建链路 | 能系统性解决所有 utility 缺失    | 范围大，风险高，不适合作为当前反馈按钮修复的首选路径 | 放弃 |

**后果**：

- 正面：反馈弹窗在桌面和移动视口下都能稳定出现在屏幕中。
- 负面/权衡：后续仍需单独整理 Tailwind 构建链路问题。
...
- `DialogOverlay` 与 `DialogContent` 的关键定位属性通过内联样式兜底，避免依赖当前不完整的 Tailwind utility 产物。
- 保持 `FeedbackDialog` 的业务逻辑和提交协议不变，避免引入不相关回归。
- 新增页面级自动化测试，验证点击反馈按钮后弹窗显示，并可通过取消按钮关闭。
- 新增手机视口下的几何位置断言，防止“弹窗存在但跑到视口外”。
...
| `feedback dialog opens from header trigger` | 手机视口下打开反馈弹窗 | `400x922` 视口 | 弹窗边界进入 viewport  | P1     | AC-04   |
...
- [ ] 修改 `src/components/ui/dialog.tsx`：为 Dialog 包装组件补充 `forwardRef`，并为关键定位样式增加兜底。
