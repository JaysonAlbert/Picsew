...
export const getDefaultFeedbackStage = (
  step: FeedbackFormProps["currentStep"],
): FeedbackStage => {
  switch (step) {
    case "processing":
      return "processing";
...
export function FeedbackForm({
  ...
  const [stage, setStage] = useState<FeedbackStage>(
    getDefaultFeedbackStage(currentStep),
  );
  ...
  const resetForm = (preserveMessages = false) => {
    ...
    setStage(getDefaultFeedbackStage(currentStep));
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      {(category === "bug_report" || category === "processing_failure") && (
        <div className="space-y-2">
          <Label htmlFor="feedback-stage">{t("feedback.stage.label")}</Label>
          <Select
            value={stage}
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger id="feedback-severity">
              <SelectValue placeholder={t("feedback.severity.placeholder")} />
            </SelectTrigger>
            <SelectContent>
